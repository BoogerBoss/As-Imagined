...
func _make_mon(mon_name: String, type1: int,
	...
	return BattlePokemon.from_species(sp, 50, BattlePokemon.NATURE_HARDY, [0, 0, 0, 0, 0, 0])  # [Flaky-suite audit fix] pinned neutral nature + zero IVs -- Section B3's PP-exhaustion-dependent discriminator predates the Nature/IV system
