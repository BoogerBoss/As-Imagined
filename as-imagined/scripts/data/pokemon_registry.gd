...
# [M24b, thin-wrapped during the M23.x-persistence-flagged consolidation pass]
# This used to be a second, independent JSON-row -> PokemonSpecies converter,
# built one day after (and unknowingly duplicating) PokemonFactory.
# build_species() — the two had already drifted apart on two real bugs
# (this function's own hand-rolled version never de-duplicated
# pokemon.json's mono-type-stored-as-[X,X] quirk, and never int()-cast most
# numeric fields read from JSON, both fixed for free by delegating).
# PokemonFactory.build_species() is the canonical implementation now — it
# has more production callers (team_storage.gd, random_team_generator.gd,
# team_builder_screen.gd) and more test coverage; this wrapper exists only
# to keep every existing caller's call site (BattlePokemon.from_trainer_mon
# chief among them) unchanged. growth_rate/learnset reasoning is now
# documented at the canonical implementation, not duplicated here.
func get_species_resource(dex_number: int) -> PokemonSpecies:
	return PokemonFactory.build_species(dex_number)
