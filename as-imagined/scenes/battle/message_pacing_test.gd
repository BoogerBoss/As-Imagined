...
func _test_wait_time_constants_are_07x_scaled() -> void:
	...
	_chk("SHORTEST is 0.268 * 0.7", is_equal_approx(BattleScreen._WAIT_TIME_SHORTEST, 0.268 * 0.7))
	_chk("SHORT is 0.536 * 0.7", is_equal_approx(BattleScreen._WAIT_TIME_SHORT, 0.536 * 0.7))
	_chk("MED is 0.804 * 0.7", is_equal_approx(BattleScreen._WAIT_TIME_MED, 0.804 * 0.7))
	_chk("LONG is 1.072 * 0.7", is_equal_approx(BattleScreen._WAIT_TIME_LONG, 1.072 * 0.7))
	_chk("waits are strictly ascending SHORTEST<SHORT<MED<LONG",
			BattleScreen._WAIT_TIME_SHORTEST < BattleScreen._WAIT_TIME_SHORT
			and BattleScreen._WAIT_TIME_SHORT < BattleScreen._WAIT_TIME_MED
			and BattleScreen._WAIT_TIME_MED < BattleScreen._WAIT_TIME_LONG)
...
func _test_text_reveal_seconds_per_char() -> void:
	...
	_chk("reveal seconds/char is ~a quarter of the source MID rate",
			is_equal_approx(BattleScreen._TEXT_REVEAL_SECONDS_PER_CHAR, 0.016745))
...
func _test_hp_drain_seconds_full_bar() -> void:
	...
	_chk("full-bar HP drain is ~0.201s (2x the real ~0.402s)",
			is_equal_approx(BattleScreen._HP_DRAIN_SECONDS_FULL_BAR, 0.201))
...
func _test_log_default_hold_is_wait_time_long() -> void:
	...
	_chk("default hold is _WAIT_TIME_LONG", is_equal_approx(beat.get("hold"), BattleScreen._WAIT_TIME_LONG))


func _test_log_custom_hold_override() -> void:
	var bs := BattleScreen.new()
	bs._log("Custom hold", BattleScreen._WAIT_TIME_SHORT)
	var beat: Dictionary = bs._pending_beats[0]
	_chk("custom hold override is respected", is_equal_approx(beat.get("hold"), BattleScreen._WAIT_TIME_SHORT))
...
func _test_flush_pending_effect_lines_pushes_beats() -> void:
	...
	_chk("flushed beat gets the default LONG hold",
			is_equal_approx(beat.get("hold"), BattleScreen._WAIT_TIME_LONG))
...
func _test_move_executed_pushes_hp_drain_beat_on_damage() -> void:
	var bs := BattleScreen.new()
	# Bare off-tree instance -- stub a real (unparented) bar so
	# _hp_fill_bar_for() resolves non-null, matching how a live scene works.
	bs._opponent_hp_fill = TextureProgressBar.new()
	...
	_chk("bar resolves to the stubbed TextureProgressBar", beat.get("bar") == bs._opponent_hp_fill)
...
func _test_move_executed_pushes_flash_beat_before_hp_drain_on_damage() -> void:
	# [Item 4] Bare off-tree instance -- stub both the bar AND the sprite so
	# _sprite_node_for()/_hp_fill_bar_for() both resolve non-null, matching
	# how a live scene works.
	var bs := BattleScreen.new()
	bs._opponent_hp_fill = TextureProgressBar.new()
	var opponent_sprite := TextureRect.new()
	bs._opponent_sprite = opponent_sprite
	var defender := _make_typed_mon("Target", TypeChart.TYPE_NORMAL, 100)
	bs._opp_party = _singles_party(defender)
	bs._player_party = _singles_party(_make_typed_mon("SomeoneElse", TypeChart.TYPE_NORMAL))
...
func _test_move_executed_no_flash_beat_when_no_damage() -> void:
	var bs := BattleScreen.new()
	bs._opponent_sprite = TextureRect.new()
...
func _test_real_battle_beat_ordering_end_to_end() -> void:
	...
	bs._bm = bm
	# A bare off-tree BattleScreen never ran _ready(), so its @onready HP-bar/
	# sprite fields are null -- stub in real (but unparented) nodes so
	# _hp_fill_bar_for()/_sprite_node_for() can both resolve non-null and the
	# "flash"/"hp_drain" beats actually get pushed, matching what a live
	# scene does.
	bs._player_hp_fill = TextureProgressBar.new()
	bs._opponent_hp_fill = TextureProgressBar.new()
	bs._player_sprite = TextureRect.new()
	bs._opponent_sprite = TextureRect.new()
